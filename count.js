/**
 * pixl77.js v2.21-user-page-session - MySQL analytics collector and dashboard tracker
 * Optimizations:
...
(() => {
  ...
  const CONFIG = Object.freeze({
    VERSION: "2.21-user-page-session",
    ...
    // null bedeutet: kein Besucher wird wegen eines unbekannten Betriebssystems verworfen.
    ACCEPTED_OS: null,
  ...
  const Utils = {
    ...
    getPageSession() {
      const now = Date.now();
      const timeoutMinutes = Math.max(1, Number(CONFIG.USER_SESSION.TIMEOUT_MINUTES) || 30);
      const recountMinutes = Math.max(1, Number(CONFIG.USER_SESSION.PAGE_RECOUNT_MINUTES) || 10);
      ...
      const reusePage = !!(
        ...
        now - Number(storedPage.startedAt) < recountMs
      );
  ...
  class PixlAnalytics {
    ...
    async init() {
      ...
      if (
        CONFIG.ACCEPTED_OS instanceof Set &&
        ...
        this.readTimeout = setTimeout(() => {
          ...
          if (!this.readNotificationSent && !this.leaveNotified) {
            this.readInterval = setInterval(
              () => this._handleReadCheckpoint(),
              CONFIG.SQL.READ_RECHECK_MS || 5000
            );
          }
        }, CONFIG.SQL.READ_DELAY_MS || 15000);
      }
    }
    ...
    _buildEventPayload(reason, title, message) {
      ...
      return {
        ...
        engagement: {
          sessionDuration,
          readingLabel: this.sessionEvents.READ ? "READ" : (this.sessionEvents.LEAVE ? "LEAVE" : "VISIT"),
          readingSeconds: this.sessionEvents.READ ? this.sessionEventTimes.READ : null,
          readingScore: score,
    ...
    _handleVisitCheckpoint() {
      ...
      if (this.notificationCount >= CONFIG.SQL.MAX_NOTIFICATIONS_PER_PAGE) return;
    ...
    _handleReadCheckpoint() {
      if (
        ...
        this.notificationCount >= CONFIG.SQL.MAX_NOTIFICATIONS_PER_PAGE
      ) return;

      const score = this.readingTracker.getScore();
      if (!this.readingTracker.hasEnoughData() || score < CONFIG.SQL.MIN_SCORE_TO_NOTIFY) {
        return;
      }
      ...
      const title = this._buildTitle();
      const message = `Reading: ${duration}s / score ${score} on ${this.context.path}`;
      const payload = this._buildEventPayload("READ", title, message);

      sendAnalyticsEvent(payload).catch(err => {
        console.error && console.error("Pixl SQL read failed:", err);
      });
    }
